...
sealed class Result<T> {
  ...
  R fold<R>({
    ...
    Failure<T>(rawError: final e, stackTrace: final st) => onFailure(e, st),
  };

  Result<R> map<R>(R Function(T value) transform) => switch (this) {
    Success<T>(value: final v) => Success(transform(v)),
    Failure<T>(rawError: final e, stackTrace: final st) => Failure(e, st),
  };
...
final class Failure<T> extends Result<T> {
  const Failure(this.rawError, [this.stackTrace = StackTrace.empty]);
  final Object rawError;
  final StackTrace stackTrace;
  @override
  T? get data => null;
  @override
  Object? get error => rawError;
  @override
  String toString() => 'Failure($rawError)';
}
